...
//check and validate the input values 
if(!$title) {
    ...
    if($thumbnail['name']) {
        $previous_thumbnail_path = ROOT_URL . './images/' . $previous_thumbnail_name;
...
if (in_array($extension, $allowed_files)) {
    ...
    $_SESSION['edit-post'] = "Couldn't update post. Thumbnail should be png, jpg, of jpeg";
}
}
}
...
$thumbnail_to_isert= $thumbnail_name ?? $previous_thumbnail_name;
$query = "UPDATE posts SET title='$title', body='$body', thumbnail='$thumbnail_to_isert', category_id=$category_id, is_featured=$is_featured WHERE id=$id LIMIT 1";
$result=mysqli_query($connection,$query);
}

if(!mysqli_errno($connection)) {
    $_SESSION['edit-post-success'] = "Post updated successfull";
}
}
